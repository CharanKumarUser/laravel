    ...
    <script>
        function enrollFace() {
            ...
            async function getCameras() {
                try {
                    ...
                    cameraSelect.innerHTML = cameras.map((cam, i) =>
                        `<option value="${cam.deviceId}">${cam.label || `Camera ${i + 1}`}</option>`).join('');
                    if (cameras.length > 0) {
            ...
            async function startCamera() {
                stopCamera();
                try {
                    CONFIG.VIDEO_CONSTRAINTS.video.deviceId = cameras[currentCameraIndex]?.deviceId ? {
            ...
            function resetGuideStroke() {
                ...
                guideCircleEl.style.transition =
                    `stroke ${CONFIG.ANIMATION_SPEED}s ease, stroke-dashoffset ${CONFIG.ANIMATION_SPEED}s ease, stroke-width ${CONFIG.ANIMATION_SPEED}s ease`;
            ...
            async function beginEnroll() {
                try {
                    ...
                    while (running) {
                        ...
                        let res;
                        try {
                            res = await human.detect(videoEl, {
                                swapRB: true
                            });
                        } catch (err) {
                            ...
                            if (descriptorStore[expected].length < 1) {
                                ...
                                if (expected === 'straight') {
                                    ...
                                    if (face.emotion && face.emotion.length > 0) {
                                        const dominant = face.emotion.reduce((max, e) => e.score > max.score ? e : max,
                                            face.emotion[0]);
