...
const SystemTheme = {
    ...
    try {
      ...
      if ($activeItem.length) {
        ...
        if (contentHeight > menuHeight && menuHeight > 0) {
          ...
          if (itemOffset < 0 || itemOffset + itemHeight > menuHeight) {
            // Position item slightly above the middle (one-third from top)
            scrollTo = itemOffset - (menuHeight / 4);
          } else {
            // Keep item in place if already visible
            scrollTo = $menu.scrollTop();
          }
          
          // Ensure scroll position is non-negative and within bounds
          scrollTo = Math.max(0, Math.min(scrollTo, contentHeight - menuHeight));
          
          // Perform smooth scroll
          $menu.animate({
            scrollTop: scrollTo
          }, 1, 'swing', () => {
            window.general?.log('Scrolled to active menu item');
          });
        }
      } else {
  ...
  setupMenuEvents() {
    const handleMenuClick = (selector, context) => {
      $(selector).on('click', e => {
        ...
        // Highlight active menu item and scroll to it
        $(`${context} li a`).removeClass('active-menu-item');
        $this.addClass('active-menu-item active');
      });
    };
  ...
  handleThemeChange() {
    ...
    this.scrollToActiveItem(); // Scroll to active item after theme change
  },
