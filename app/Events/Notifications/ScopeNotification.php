...
/**
 * ScopeNotification event broadcasts notifications to users within specific scopes.
 *
 * This event is triggered when a notification is sent to users associated with specific
 * scopes, broadcasting details such as title, message, and custom HTML content.
 * It uses the options array for HTML, image, and sender data.
 */
class ScopeNotification implements ShouldBroadcast
{
    ...
    /**
     * Create a new ScopeNotification event instance.
     *
     ...
     * @param string $category Notification category (e.g., 'policy', 'event').
     * @param string $type Notification type (e.g., 'update', 'announcement').
     ...
     *
     * Maps each user ID to a private channel (user.{userId}).
     *
     ...
     * The html field contains resolved placeholder content.
     *
