...
use Illuminate\Support\Facades\{Auth, Config, DB, Hash, Validator, Cache};
...
class SaveAddCtrl extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $cacheKey = "users_{$userData->business_id}_set";
            ...
            switch ($reqSet['key']) {
                case 'open_profile_edit':
                    if ($type == 'firstform') {
                        ...
                        $result = Data::update($system, 'user_info', $userinfo, [
                            ['column' => 'user_id', 'operator' => '=', 'value' => $userData->user_id]
                        ]);
                        if (!$result['status']) {
                            return ResponseHelper::moduleError('User Info Update Failed', $userInfoResult['message'], 400);
                        }
                        ...
                        foreach ($emergency as $index => $entry) {
                            ...
                            //     return response()->json([
                            //         'status' => 'error',
                            //         'message' => "Validation failed for emergency contact entry #" . ($index + 1),
                            //         'errors' => $validator->errors(),
                            //     ], 422);
                            // }
                        }
                        ...
                        if ($system != "central") {
                            $usersResult = Data::update('central', 'users', $usersData, [['column' => 'users.user_id', 'operator' => '=', 'value' => $userData->user_id]]);
                        }
                        if (!$usersResult['status']) {
                            return ResponseHelper::moduleError('User Update Failed', $usersResult['message'], 400);
                        }
