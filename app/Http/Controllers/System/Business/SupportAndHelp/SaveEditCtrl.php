            ...
            // Handle different configuration keys
             switch ($reqSet['key']) {
                ...
                $validator = Validator::make($request->all(), [
                    'issue_scope'      => 'nullable|in:Internal,External',
                ...
                if ($validated['issue_scope'] == 'External') {
                    $supportData = BusinessDB::table('supports')
                        ->where($reqSet['act'], $reqSet['id'])
                        ->first();

                    if ($supportData) {
                        $supportData = (array) $supportData;
                        $exists = CentralDB::table('supports')
                            ->where($reqSet['act'], $reqSet['id'])
                            ->exists();
                        if (!$exists) {
                            unset(
                                $supportData['id'],
                                $supportData['deleted_at'],
                                $supportData['restored_at'],
                                $supportData['delete_on'],
                                $supportData['created_at'],
                                $supportData['updated_at']
                            );
                            $supportData['created_at'] = now();
                            $supportData['issue_scope'] = 'Support';
                            $result = Data::insert('central', $reqSet['table'], $supportData, $reqSet['key']);
                            if (!$result['status']) {
