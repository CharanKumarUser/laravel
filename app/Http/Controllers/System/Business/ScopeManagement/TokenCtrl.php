...
class TokenCtrl extends Controller
{
    ...
    public function index(Request $request, array $params)
    {
        try {
            ...
            $baseView = 'system.central.scope-management';
            // Default View
            $viewPath = 'errors.404';
            $data = [
                'status' => true,
            ...
            switch ($action) {
                case 'page':
                    $viewPath = $baseView . '.scope-view';
                    $scopeIdsArr = Scope::getChilds($token);
                    $scopeIds = implode('-', array_keys($scopeIdsArr));
                    $system = Skeleton::authUser('system');
                    $result = Data::fetch($system, 'scopes', ['where' => ['scope_id' => $token]]);
                    $scopeInfo = $result['data'][0] ?? null;
                    $data = [
                        'status' => true,
                        'scope_id' => $token,
                        'scope_info' => $scopeInfo,
                        'child_arr' => $scopeIdsArr,
                        'child_str' => $scopeIds,
                    ];
                    break;
                default:
