...
class FormCtrl extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $validated = [];
            $title = 'Success';
            $message = 'ScopeManagement data saved successfully.';
            /****************************************************************************************************
             *                                                                                                  *
            ...
            switch ($reqSet['key']) {
                case 'ScopeManagement_entities':
                    ...
                    $title = 'Entity Added';
                    $message = 'ScopeManagement entity configuration added successfully.';
                    break;
                default:
            ...
            // Generate response
            return response()->json(['status' => $result['status'], 'reload_table' => $reloadTable, 'reload_card' => $reloadCard, 'reload_page' => $reloadPage, 'hold_popup' => $holdPoup, 'token' => $reqSet['token'], 'affected' => $result['status'] ? $result['id'] : '-', 'title' => $result['status'] ? $title : 'Failed', 'message' => $result['status'] ? $message : $result['message']]);
        } catch (Exception $e) {
