...
use App\Facades\{Skeleton, CentralDB, Developer};
...
class NavCtrl extends Controller
{
    ...
    public function index(Request $request, array $params)
    {
        try {
            ...
            $baseView = 'system.central.support-and-help';
            ...
            switch ($viewName) {
                ...
                // --- Summary counts
                $counts = CentralDB::table('supports')
                    ->select(
                        CentralDB::raw("COUNT(*) as total_tickets"),
                        CentralDB::raw("SUM(CASE WHEN issue_status = 'Open' THEN 1 ELSE 0 END) as open_tickets"),
                        CentralDB::raw("SUM(CASE WHEN issue_status IN ('Resolved','Closed') THEN 1 ELSE 0 END) as solved_tickets"),
                        CentralDB::raw("SUM(CASE WHEN issue_status = 'In Progress' THEN 1 ELSE 0 END) as pending_tickets")
                    )
                    ->where('is_active', 1)
                    ->where('user_id', Skeleton::authUser()->user_id)
                    ->first();

                    $categories = CentralDB::table('supports')
                        ->select('issue_category', CentralDB::raw('COUNT(*) as total'))
                        ->where('is_active', 1)
                    ...
                    $priorityCounts = CentralDB::table('supports')
                        ->select('issue_priority', CentralDB::raw('COUNT(*) as total'))
                        ->where('is_active', 1)
                        ->where('user_id', Skeleton::authUser()->user_id)
